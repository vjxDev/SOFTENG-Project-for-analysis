...
sub mysystem($@);
sub find_file($);
sub read_DSECT_types();

sub ::D { require Data::Dumper; $Data::Dumper::Indent += 0;
          local $Data::Dumper::Indent = 1; Data::Dumper::Dumper(@_) }
...
if (!$dos) {
  ...
  $chicken_exec = "$FermaT/chicken/$arch/scmfmt";
...
END
    $cmds =~ s/\(load /\(loadq /g;
    my $tmpfile1 = tmpfile($cmds);
    #my $tmpfile2 = tmpfile(qq[(car (cons "" (loadq "$tmpfile1")))]);
    my $tmpfile2 = tmpfile(qq[(begin (loadq "$tmpfile1") "")]);
    #system qq[echo '(loadq "$tmpfile")' | $bigloo_exec];
    mysystem "$bigloo_exec $tmpfile2";
    unlink($tmpfile);

  } elsif (defined($ENV{SCM_IMPL}) && ($ENV{SCM_IMPL} eq "chicken")) {
    # chicken compiled version
    $defines =~ s/define/set!/g;
    $cmds = <<END;
...
END
    ...
    mysystem "$chicken_exec -i -s $tmpfile";
    unlink($tmpfile);
...
# or:
# module {CICS|CICSEIB|CICSDATA|DATA|E15,E35}[,number-of-pars]
...
#   via this register)
#
# module could be of the form module.entry
#
# For sort exits the format is:
# [E15|E35],in-rec-len,[FB|VB],out-rec-len,[FB|VB]
#
...
sub parallel_check($@) {
  ...
  if ($ENV{FermaT_parallel} && @args > 1) {
    ...
    my $opts = "";
    my $nice = 4;
    my $cores = `parallel --number-of-cores`;
    my $par_ver = `parallel --version`;
    if ($par_ver =~ /GNU parallel (\d+)/) {
      $par_ver = $1;
    } else {
      $par_ver = 0;
    }
    $opts = "--use-cores-instead-of-threads"
      if ($cores > 30) && ($par_ver >= 20201122);
    $nice = 20 if $cores > 30;
    open(PAR, "|nice -n $nice parallel $opts $script")
      or die "Cannot exec parallel: $!\n";
    ...
    print PAR map { "$_\n" } @args;
    close(PAR);
    if (defined($ENV{FermaT_parallel_post})) {
      my $cmd = $ENV{FermaT_parallel_post};
      undef $ENV{FermaT_parallel_post};
      print "Parallel commands have finished, running post:\n$cmd\n";
      system "$cmd" if $cmd =~ /\S/;
    }
    exit(0);
  }
...
# Read the file DSECT-types.txt, return a hash table ref
# containing just the global DSECTs so that we can use
# if ($tab{$dsect}) to check the status of a DSECT.
# A DESCT can be recorded in the table as local (type L)
# to stop it from later being overwritten as global.

sub read_DSECT_types() {
  my %type = ();
  my $file = "DSECT-types.txt";
  my $path = "";
  my $default =  $::FermaT . $::ds . "config" . $::ds . $file;
  if ($path = find_file($file)) {
    # found options in current dir or an ancestor
  } elsif (-f $default) {
    $path = $default;
  }
  # It is OK if there is no types file:
  if (-f $path) {
    open(my $in, $path) or die "Can't read types file $path: $!\n";
    while (<$in>) {
      next unless /^(\S+)\s+([LG])\s*$/;
      next unless $2 eq "G";
      $type{$1} = $2;
    }
    close($in);
