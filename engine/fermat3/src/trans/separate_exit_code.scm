...
(define (/foreach-separate_exit_code-1 //Depth //A/S_/Type)
 (cond
  ((and (not (@Failed?)) (= //Depth 1) (equal? (@Gen_TVs (@I) //A/S_/Type) (list 1)) (not (equal? (@Stat_Types (@I)) (list //T_/Exit))))
   (cond
    ((@Gen_Reducible? (@I) //A/S_/Type)
     (@Pass))
    (#t
...
(define (/foreach-separate_exit_code-2 //Depth //A/S_/Type)
 (cond
  ((and (= (gen-length (@Gen_TVs (@I) //A/S_/Type)) 1) (>= (car (@Gen_TVs (@I) //A/S_/Type)) //Depth))
   (set! /body (@I))
   (set! /count (+ /count 1))
  ...
  ((and (= (gen-length (@Gen_TVs (@I) //A/S_/Type)) 1) (>= (car (@Gen_TVs (@I) //A/S_/Type)) //Depth))
   (cond
    ...
    (#t
     ...
     (cond
      ((> //Depth 1)
       (set! /body (@Cs (@Increment (@I) (@AS_Type) (- 1 //Depth) 1))))
      (#t
       (set! /body (@Cs (@I)))))
     (set! /exit (@Make //T_/Exit //Depth '()))
     (set! /guards (cons (@Make 7 '() (list (@Make 313 '() (list (@Var_To_Expn /flag) (@Var_To_Expn /n))) (@Make 17 '() /body))) /guards))
     (@Paste_Over (@Make 17 '() (list (@Make 110 '() (list (@Make 6 '() (list (@Expn_To_Var /flag) (@Var_To_Expn /n))))) /exit))))))
...
(define (@SEC_Split //L //S1-par //S2-par)
 (let ((//S2-save //S2)
  ...
  (let ((//L1 (cdr (reverse //L)))
        (//L2 (list (last-1 //L))))
   (while (and (not (null? //L1)) (@Gen_Proper? (car //L1) (@AS_Type))) 
    (begin
