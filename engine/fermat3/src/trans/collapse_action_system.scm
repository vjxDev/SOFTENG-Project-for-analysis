...
(define (/foreach-collapse_action_system-6 //Depth //A/S_/Type)
 (cond
  ...
  ((= (@ST (@I)) //T_/Call)
   (set! /next (@Make //T_/Number (- (@V (@I))) '()))
   (@Paste_Over (@Make //T_/Exit (+ 1 //Depth) '()))
   (@Paste_Before (@Make 110 '() (list (@Make 6 '() (list (@Expn_To_Var /action) (@Var_To_Expn /next)))))))))
...
(define (@Collapse_Action_System_Code //Data)
 (let ((/old_size (@Total_Size (@Parent)))
       ...
       (/posn_n (@Posn_n)))
  (set! /posn (@Posn))
  ; Collapse_Action_System_Simple may create procs in an outer WHERE. 
  ; So to compute the size change we need to process the action system 
  ; in isolation 
  (@Edit_Parent)
  ...
  (@Undo_Edit)
  (@Down_To /posn_n)
  (display-list "Old size = " /old_size)
  (display-list "New size = " /new_size)
  (display-list "Old posn = " /posn)
  (display-list "New Posn = " (@Posn))
  (cond
   ((<= /new_size (* 3 /old_size))
    (@Collapse_Action_System_Simple //Data))
   (#t
...
(define (@Collapse_Action_System_New2 //Data)
 (let ((//N-save //N))
  ...
  (let ((//A/S_/Name (@V (@Get_n (@I) 1)))
        ...
        (/next-save /next)
        (/body-save /body))
   (set! /action (@Make //T_/Variable (@Make_Name (string-append (@N_String //A/S_/Name) "_ACTION")) '()))
   (set! //Names (make-vector-eval (+ //N 1) '()))
   (set! /next '())
   (set! /body '())
   (cond
    ((@Starts_With? //A/S_/Name "_")
     (set! /action (@Make //T_/Variable (@Make_Name "action") '())))
    (#t
     (@Paste_After (@Make //T_/Comment (string-append (string-append "* #SMLDATA# " (@N_String (@V /action))) " N 4") '()))))
   (@Edit)
   ; Calculate Bodies, Names, Name2Num 
   ; Hash table Name2Num maps action names (keys) to action numbers   
   (let ((/-result- (@FD_Init  //N //Bodies //Names //Name2/Num)))
   ...
   (for /i 1 //N 1 
    (begin
     ...
     (set! /body (@Cs (@Program)))
     (set! /next (@Make //T_/Number /i '()))
     (set! /guards (cons (@Make 7 '() (list (@Make 313 '() (list (@Var_To_Expn /action) (@Var_To_Expn /next))) (@Make 17 '() /body))) /guards))))
   (set! /guards (cons /%const__collapse_action_system__5 /guards))
   (set! /body (@Make //T_/Cond '() (reverse /guards)))
   (set! /next (@Make //T_/Number //Starting_/Action '()))
   (@Undo_Edit)
   (@Paste_Over (@Make 110 '() (list (@Make 6 '() (list (@Expn_To_Var /action) (@Var_To_Expn /next))))))
